...
{
    public enum Polarity { plus, minus, metal };
...
///////////////////////////////////////////////////////////////
    ...
    private void OnTriggerStay2D(Collider2D other) 
    {
        ...
        if (result)
        {
            MagneticObject mo = other.GetComponent<MagneticObject>();
            PlayerController pc = other.GetComponent<PlayerController>();
            Rigidbody2D rb = other.GetComponent<Rigidbody2D>();
            bool invert = false;
            if(mo)
            {
                if(!mo.IsConducting()) return;
                else invert = (mo.Polarity == polarity) ? true : false;  
            }
            if(pc)
            {
                if(!pc.IsConducting())
